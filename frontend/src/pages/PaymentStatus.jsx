...
const PaymentStatus = () => {
    ...
    useEffect(() => {
        api.post('/payments/status', { txnid: txnid })
            .then(response => {
    ...
    return (<>
        <div className='flex gap-8 rounded-lg items-center w-full md:h-[calc(100vh-10rem)] justify-start'>
            <div className='flex flex-col rounded-lg p-8 shadow-lg border-2 h-full w-full bg-container justify-start items-center md:p-16'>
                {!loading ?
                    !paymentDetails.error
                        ...
                        <>
                                ...
                                </p>
                                {
                                    paymentDetails.status === 'PAYMENT_SUCCESS' ?
                                        <p className='text-green-500'>
                                            Sponsorship amount successfully recieved by CCS
                                        </p>
                                        :
                                        <p className='text-red-500'>
                                            Sponsorship amount not recieved by CCS
                                        </p>
                                }
                            </div>
                            ...
                            <div className='flex flex-col gap-4 text-lg'>
                                ...
                                <div>
                                    <span className='font-bold'>Paid Amount:</span> INR {paymentDetails.amount}/-
                                </div>
                                <div>
                                    <span className='font-bold'>Payment Status:</span> {paymentDetails.status}
                                </div>
                                <div>
